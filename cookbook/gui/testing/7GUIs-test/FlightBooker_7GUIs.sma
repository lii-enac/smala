...
FlightBooker_7GUIs ()
{
    ...
    B.click -> (this) {   // [7GUIs] When clicking B...
        if (this.is_return_flight) {
            this.tp.input = "You have booked a " + this.C.value + " on " + this.T1.text + " return on " + this.T2.text
        }
        else {                                                      
            this.tp.input = "You have booked a " + this.C.value + " on " + this.T1.text      // [7GUIs] ...a message is displayed informing the user of his selection 
    ...
    // # Bonus
    T1.next -> (this) {
        if (this.is_return_flight == 1) {
            notify this.T2.activate
        }
        else {
            notify this.B.select
        }
